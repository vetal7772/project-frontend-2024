...
const PROFESSION_ARRAY = ['ROGUE', 'SORCERER', 'CLERIC', 'PALADIN', 'NAZGUL', 'WARLOCK', 'DRUID'];
        ...
        players.forEach((player) => {
            htmlRows +=
                ` <tr class="row" data-account-id="${player.id}">
                    <td class="cell cell-min">${player.id}</td>
...
function updatePaginationButtons() {
    ...
    const childButtonCount = $buttonContainer.children.length;
    ...
    if (childButtonCount !== 0) {
        Array.from($buttonContainer.children).forEach(node => node.remove());
    }

    $buttonContainer.insertAdjacentHTML("beforeend", paginationButtonsHtml);
    Array.from($buttonContainer.children).forEach(button => button.addEventListener('click', onPageChange));
...
function createSelectOptions(optionsArray, defaultValue) {
    ...
    optionsArray.forEach(option => {
        optionHtml += `<option ${defaultValue === option && 'selected'} value="${option}">${option}</option> `;
    });
    return optionHtml;
}
